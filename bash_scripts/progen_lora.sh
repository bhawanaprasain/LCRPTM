...
mkdir -p training_logs/progen_${progen_model_size}/${ptm_alias}

nohup python3 -u src/progen/lora_ft.py > training_logs/progen_${progen_model_size}/${ptm_alias}/motif_marker_${motif_mode}_window_${window_size}_target_modules_${target_modules}_lora_rank_${rank}_lora_alpha_${lora_alpha}_num_of_epochs_${num_of_epochs}.log 2>&1 &
